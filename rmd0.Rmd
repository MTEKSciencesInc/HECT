...
The HECT simulator is an application designed to explore the various features and utilities of Bayesian Response Adaptive Randomized trial designs (BRAR). For a given set of design components specified by the user, data can be generated for a single BRAR trial with an embedded Bayesian analysis. Single trial simulation provides a better understanding of the progression of the trial with the adaptive component under different scenarios. In addition, under the `Trial design properties` tab, simulation studies are enhanced to investigate statistical properties including power and type I error rate. The HECT simulator is designed to be accessible to non-statisticians such that an intuitive understanding of the adaptive design is enough to use the application for planning clinical trials. However, a detailed description of the underlying assumptions and statistical models is provided in the following sections.
