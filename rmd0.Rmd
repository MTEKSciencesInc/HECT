...
**Comparison with RCT:** It is often of interest to compare an adaptive design to a conventional design with fixed sample size. In the HECT simulator the properties of the design specified by the user are compared to a design with a fixed sample size set at the maximum total sample size, balanced randomization and no interim looks. Note that, to assure fairness of the comparison, the final analysis for the simulated conventional trial is a Bayesian analysis with the same statistical model that is used for the adaptive trial. The comparisons are presented in form of graphs of power, type I error rate and average cost for the two trial designs.
